...
heights %>%
  filter(sex == "Female") %>%
  summarise(median = median(height), mimimum = min(height), maximum = max(height))
```
...
heights %>%
  group_by(sex) %>%
  summarize(average = mean(height), standatd_deviation = sd(height))
```
...
```{r}
ref <- NHANES %>% filter(AgeDecade == "20-29") %>%
  summarize(minimum= min(NHANES$BPSysAve, na.rm = TRUE), maximum = max(NHANES$BPSysAve, na.rm = TRUE))
head(ref)
```
...
NHANES %>%
  ...
  summarize(average= mean(NHANES$BPSysAve, na.rm = TRUE), standard_deviation = sd(NHANES$BPSysAve, na.rm = TRUE)) %>% head()
```
...
NHANES %>%
  ...
  group_by(Race1) %>%
  summarize(average =mean(NHANES$BPSysAve, na.rm = TRUE)) %>%
  arrange(average)
```
##5.10 Tibbles
...
```{r}
a <- function(dat){x <- exp(mean(log(dat$population)))} %>%
data_frame(x)
murders %>%
  do(a(.))
```
