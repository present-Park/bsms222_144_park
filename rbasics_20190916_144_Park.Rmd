...
```
How can we compute  Sn for various values of  n say  n=1,…,25? Do we write 25 lines of code calling compute_s_n? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of  
...
```
Here is the for-loop we would write for our Sn exmple:
```{r}
m <- 25
s_n <- vector(length = m)
for(n in 1:m){
...
```
Now we can creat a plot to search for a pattern
```{r}
n <- 1:m
plot(n, s_n)
```
